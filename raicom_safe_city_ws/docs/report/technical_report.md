...
为便于 YOLO 识别并缩小仿真与真实视觉之间的差距，项目已将早期用于流程验证的纯色几何体升级为真实照片 billboard。垃圾桶、人偶和楼宇目标均使用 `平安城市/` 原始素材生成贴图，并以竖直薄板方式放置在 Gazebo 场景中。这样机器人相机看到的不再只是颜色圆柱体或长方体，而是带真实图片纹理的目标外观。目标类别与终端中文输出严格统一，避免出现非比赛口径的历史类别名称。
...
| `scripts/build_billboard_yolo_dataset.py` | 使用真实贴图合成 billboard 训练数据 |
| `scripts/train_gazebo_yolo_model.sh` | 训练并导出最终 YOLO 权重 |

在完成真实照片 billboard 场景升级后，项目基于当前贴图目标又进行了短轮次适配微调。该训练用于让模型从“彩色几何体 + 原始图”进一步适配到“Gazebo 相机中的真实照片贴图目标”。本轮 billboard 适配验证结果如下：
...
经过该轮适配后，照片 billboard 版本 Gazebo 世界中三大巡检区域均恢复为 YOLO 主识别输出。需要说明的是，该指标主要反映当前仿真贴图场景的适配效果，并不能等同于真实赛场泛化能力。若后续迁移到真实摄像头，仍需补充真实拍摄素材与更高质量边界框标注。
...
- 当前 Gazebo 目标已升级为真实照片 billboard，但姿态、背景和光照仍较规整。后续可增加目标倾斜、部分遮挡、随机光照和更复杂背景，提高模型鲁棒性。
